...
#noise_type_array = ['white_noise','wind_noise', 'rain_noise']
noise_type_array = ['pink_noise']
...
for iNoise in range(nNoise):
    ...
    noise_level_array = np.array([-20])
    #noise_level_array = np.array([-20,-10,0,10,20])
    ...
    for iLevel in range(nLevel):
        ...
        for iSeg in range(nSeg):
            ...
            for iPer in range(nPer):
                ...
                win_time_array = np.array([ 0.1 ])* fs
                ...
                for iTime in range(nTime):
                    ...
                    for iStep in range(nStep):
                        ...
                        train_path = 'D:/Project/Segmentaion_revise1/feat_frame_noise' + \
                          '/' + select_noise_type + '_' + str(select_noise_level) + '/' + select_segment + \
                         '/training_' + str(select_percent) + \
                        ...
                        for iTrain in np.arange(0, nTrain, 2):
                            ...
                            b = os.path.getsize(train_csv_path)
                            if b != 0:                                                           
                        ...
                        test_path = 'D:/Project/Segmentaion_revise1/feat_frame_noise' + \
                          '/' + select_noise_type + '_' + str(select_noise_level) + '/' + select_segment + \
                            '/testing_' + str(select_percent) + \
                            '/win_len_' + str(int(select_win_time)) + '_win_over_' + str(select_win_step)
                        ...
                        for iTest in np.arange(0, nTest, 2):
                            ...
                            b = os.path.getsize(test_csv_path)
                            if b != 0:                                                           
                        ...
                        #hist_value = np.histogram(train_label, bins=24)
                        ...
                        # save path
                        save_folder = './out_label_frame_noise/' +  select_segment + '/training_' + str(select_percent) + \
                            '/' + select_noise_type + '_' + str(select_noise_level) + \
                            '/win_len_' + str(int(select_win_time)) + '_win_over_' + str(select_win_step)
                                                                  
                        make_sure_path_exists(save_folder)
                        save_path =save_folder + '/outlabel_cv2.csv'
                        ...
                        save_path_performance = save_folder + '/performance_cv2.csv'
