...
fs = 44100
#===========================#
noise_type_array = ['pink_noise', 'white_noise', 'wind_noise', 'rain_noise']
nNoise = len(noise_type_array)
for iNoise in range(nNoise):
    
    select_noise_type = noise_type_array[iNoise]

    noise_level_array = np.array([-20,-10,0,10])
    nLevel = len(noise_level_array)
    for iLevel in range(nLevel):
    
        select_noise_level =  noise_level_array[iLevel]

        norm_type_array = np.array([2])
        nNorm = len(norm_type_array)
        for iNorm in range(nNorm):
            
            select_norm = norm_type_array[iNorm]
        
            label_type_array = ['mid', 'strong', 'weak']
            ...
            for iType in range(nType):
                ...
                for iPerc in range(nPerc):
                    ...
                    #win_size_array = np.array([0.01, 0.05, 0.1]) # samples
                    #win_size_array = np.array([0.02, 0.05, 0.1, 0.2, 0.5]) # seconds
                    ...
                    for iWin in range(nWin):
                        ...
                        for iOver in range(nOver):
                            ...
                            #===========================#
                            train_path = baseFolder + '/raw_data_sliding_noise/' + str(select_noise_level) + \
                            '/noise_data_' + select_noise_type +'/result_all_percent_' + str(select_percent) + \
                            '_winsize_' + str(int(select_win_size*fs)) + '_winover_' + str(select_win_over) + \
                            '/training_raw_data_sliding_window.csv'
                                     
                            test_path = baseFolder + '/raw_data_sliding_noise/' + str(select_noise_level) + \
                            '/noise_data_' + select_noise_type +'/result_all_percent_' + str(select_percent) + \
                            '_winsize_' + str(int(select_win_size*fs)) + '_winover_' + str(select_win_over) + \
                            '/testing_raw_data_sliding_window.csv'
                            ...
                            save_folder_final = baseFolder + '/python_LSTM_noise/' + label_type + '_normal_final_' + \
                                str(select_norm) + '/result_all_percent_' + str(select_percent) + '_winsize_' + \
                                str(int(select_win_size*fs)) + '_winover_' + str(select_win_over) + \
                                '/' + select_noise_type + '_' + str(select_noise_level)
